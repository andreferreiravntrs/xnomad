...
const estimatedTransaction = (unitType, bookingStart, bookingEnd, unitPrice, dynamicPricing, quantity) => {
  const now = new Date();
  ...
  if (dynamicPricing) {
    if (unitCount < 7) {
      unitPrice = dynamicPricing.priceLess7Days;
    } else if (unitCount >= 7 && unitCount < 30) {
      unitPrice = dynamicPricing.priceMore7Days;
    } else if (unitCount >= 30) {
      unitPrice = dynamicPricing.priceMore30Days;
    }
  }
...
const EstimatedBreakdownMaybe = props => {
  const { unitType, unitPrice, dynamicPricing, startDate, endDate, quantity } = props.bookingData;
  ...
  const tx = estimatedTransaction(unitType, startDate, endDate, unitPrice, dynamicPricing, quantity);
  return (
    <BookingBreakdown
