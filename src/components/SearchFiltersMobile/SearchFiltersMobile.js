...
class SearchFiltersMobileComponent extends Component {
  constructor(props) {
    ...
    this.initialAreaRangeValue = this.initialAreaRangeValue.bind(this);
  ...
  initialAreaRangeValue(paramName) {
    const urlQueryParams = this.props.urlQueryParams;
    const area = urlQueryParams[paramName];
    const valuesFromParams = !!area ? area.split(',').map(v => Number.parseInt(v, RADIX)) : [];

    return !!area && valuesFromParams.length === 2
      ? {
  ...
  render() {
    ...
    const initialAreaRange = this.initialAreaRangeValue(areaFilter.paramName);

    const areaFilterElement = areaFilter ? (
      <AreaFilter
        id="SearchFiltersMobile.areaFilter"
...
SearchFiltersMobileComponent.propTypes = {
  ...
  areaFilter: propTypes.filterConfig,
